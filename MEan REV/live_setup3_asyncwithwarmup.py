...
class stock:
    ...
    async def BuyNSE_SellBSE(self):
        return await asyncio.gather(
            self.fapi.place_order("NSE", self.NSE, 1, self.NSEPrice + self.tol, "B"),
            self.fapi.place_order("BSE", self.BSE, 1, self.BSEPrice - self.tol, "S")
        )

    async def SellNSE_BuyBSE(self):
        return await asyncio.gather(
            self.fapi.place_order("NSE", self.NSE, 1, self.NSEPrice - self.tol, "S"),
            self.fapi.place_order("BSE", self.BSE, 1, self.BSEPrice + self.tol, "B")
        )
...
async def periodic_order_placer(fapi, exchange, symbol, qty, price, txn_type, interval_sec):
    while True:
        ...
        logging.info(f"Placed {txn_type} order for {symbol} on {exchange} every {interval_sec} sec")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        await asyncio.gather(
            ...
            periodic_order_placer(
                ...
                symbol="TATAMOTORS",
                qty=1,
                price=0,  # Static price; can be made dynamic
                txn_type="B",
                interval_sec=3
            )
        )
