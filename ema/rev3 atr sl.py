...
for i in range(0,len(datetime_objects)):
    ...
    bar15_length.append(high_values[i]-low_values[i])
    start=max(0,len(bar15_length)-1-atrlength)
    window=bar15_length[start:len(bar15_length)]
    ...
    if position=="Long":
        if (close_values[i]-atr)>stoploss:
            stoploss=close_values[i]-atr
    ...
    if bar15==15:
        ...
        if seek_status=="Empty":
            if curr15_low>ema_value:
                ...
                # temp_stoploss=close_values[i]+(atr*atr_increment)
...
# plt.plot(net_data_time,atrlst,color='olive',linestyle='-')
