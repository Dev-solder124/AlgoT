...
virtual_target_increment=0.2
increment_buffer=0
increment_term=1
increment_power=0.05
...
for i in range(0,end_day-start_day):
    if position=="Long":
        ...
        if(close_values[i]>=virtual_target):
            increment_term+=1
            virtual_target+=virtual_target_increment*buffer
            stoploss+=(buffer*(math.log(increment_term**increment_power)))
        ...
        if ((close_values[i]<=stoploss) or (datetime_objects[i].hour==15 and datetime_objects[i].minute==29)):
            ...
            plot_SL.append(stoploss)
            plot_T.append(virtual_target)
            plot_times.append(datetime_objects[i])
            if(close_values[i]-entry<=0):
                plt.plot(plot_times,plot_bar,color='r',marker='^')
                plt.plot(plot_times,plot_SL,color='m')
                plt.plot(plot_times,plot_T,color='y')
                Ltrade+=1
            ...
            plot_T.clear()
            if print_processs:
                print("SL hit, Long exit at",close_values[i],"with",close_values[i]-entry,"points")
            ...
            plot_times.clear()
            if print_processs:
                print("Target hit, Long exit at",close_values[i],"with",close_values[i]-entry,"points")
            net+=close_values[i]-entry
    ...
    if position=="Short":
        ...
        if ((close_values[i]>=stoploss) or (datetime_objects[i].hour==15 and datetime_objects[i].minute==29)):
            ...
            plot_times.clear()
            if print_processs:
                print("SL hit, Short exit at",close_values[i],"with",-close_values[i]+entry,"points")
            net+=-close_values[i]+entry
            ...
            plot_times.clear()
            if print_processs:
                print("Target hit, Short exit at",close_values[i],"with",-close_values[i]+entry,"points")
            net+=-close_values[i]+entry
    ...
    if bar==15:
        ...
        if position=="Long seeking":
            if close_values[i]>=entry_target:
                ...
                virtual_target=entry+virtual_target_increment*buffer
                plot_T.append(virtual_target)
                if print_processs:
        ...
        if position=="Empty":
            if curr_low>ema_values[i]:
                ...
                if print_processs:
                    print(position,"SL",stoploss,"GAP:",gap)
