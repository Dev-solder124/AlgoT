...
for i in range(0,len(datetime_objects)):
    ...
    if position=="Long":
        ...
        if ((low_values[i]<=stoploss) or (datetime_objects[i].hour==15 and datetime_objects[i].minute==29)):
            position="Empty"
            plot_bar.append(close_values[i])
            plot_times.append(datetime_objects[i])
            if(low_values[i]-entry<=0):
                plt.plot(plot_times,plot_bar,color='r',marker='^')
                Ltrade+=1
                Lpoints+=low_values[i]-entry
            else:
            ...
            plot_times.clear()
            if print_processs:
                print(datetime_strings[i],"Long exit at",close_values[i],"with",low_values[i]-entry,"points")
            ...
            plot_times.clear()
            if print_processs:
                print(datetime_strings[i],"Long exit at",high_values[i],"with",close_values[i]-entry,"points")
            net+=high_values[i]-entry

            Ptrade+=1
            Ppoints+=high_values[i]-entry
    ...
    if position=="Short":
        if ((high_values[i]>=stoploss) or (datetime_objects[i].hour==15 and datetime_objects[i].minute==29)):
            position="Empty"
            plot_bar.append(close_values[i])
            plot_times.append(datetime_objects[i])
            if(-high_values[i]+entry<=0):
                plt.plot(plot_times,plot_bar,color='r',marker='v')
                Ltrade+=1
                Lpoints+=-high_values[i]+entry
            else:
            ...
            plot_times.clear()
            if print_processs:
                print(datetime_strings[i],"Short exit at",high_values[i],"with",-close_values[i]+entry,"points")
            net+=-high_values[i]+entry
