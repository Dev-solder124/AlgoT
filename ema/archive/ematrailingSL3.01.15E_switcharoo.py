...
initial_target_increment=3

target_increment=0.35
stoploss_increment=0.3
...
for i in range(0,end_day-start_day):
    if position=="Long":
            if ((close_values[i]<=stoploss) or (datetime_objects[i].hour==15 and datetime_objects[i].minute==29)):
                ...
                target+=buffer*target_increment
                stoploss+=buffer*stoploss_increment
            
                plot_bar.append(close_values[i])
                plot_times.append(datetime_objects[i])
                if print_processs:
                    print("Target hit, new target:",target,"SL:",stoploss)
    ...
    if position=="Short":
        if ((close_values[i]>=stoploss) or (datetime_objects[i].hour==15 and datetime_objects[i].minute==29)):
            ...
            plot_times.clear()
            if print_processs:
                    print("Short exit at",close_values[i],"with",entry-close_values[i],"points")
            net+=entry-close_values[i]
            if ((-close_values[i]+entry)>0):
                Ptrade+=1
            else:
                Ltrade+=1
    ...
    if bar==15:
        ...
        if position=="Long seeking":
            if close_values[i]>=entry_target:
                ...
                if buffer==0:
                    buffer=0.5
                target=entry+initial_target_increment*buffer  

            elif close_values[i]<=stoploss:
                position="Empty"
                if print_processs:
                        print("Long abandoned, Short Initiated")
                position="Short"
                plot_bar.append(close_values[i])
                plot_times.append(datetime_objects[i])
                entry=close_values[i]
                buffer=abs(entry_target-close_values[i])
                if buffer==0:
                    buffer=0.5
                target=entry-initial_target_increment*buffer                    
        ...
        if position=="Short seeking":
            if close_values[i]<=entry_target:
                ...
                position="Empty"
                if print_processs:
                        print("Short abandoned, Long initiated")
                position="Long"
                plot_bar.append(close_values[i])
                plot_times.append(datetime_objects[i])
                entry=close_values[i]
        ...
        if position=="Empty":
            if curr_low>ema_values[i]:
                ...
                stoploss=curr_high
                buffer=abs(stoploss-bar_closevalue)
                if buffer==0:
                    buffer=0.5
                if print_processs:
                ...
                stoploss=curr_low
                buffer=abs(bar_closevalue-stoploss)
                if buffer==0:
                    buffer=0.5
                if print_processs:
                    print(position,"entry=",entry,"buffer:",buffer,"SL",stoploss,"Tgrt",target, "GAP:",gap)
...
plt.plot(datetime_objects,net_data,color='y')
