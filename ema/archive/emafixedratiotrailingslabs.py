...
half_target_increment=9
half_target_status=0
half_target_L=0
half_target=0
half_loss_increment=0
...
for i in range(0,end_day-start_day):
    if position=="Long":
        ...
        if(close_values[i]>=half_target):
            half_target_status=1
            stoploss+=buffer*half_loss_increment
    ...
    if position=="Long seeking":
        if close_values[i]>=entry_target:
            ...
            half_target=entry+half_target_increment*buffer  
...
print("half target hit and l",half_target_L)
plt.show()