...
# Correct instrument keys from the API response
INSTRUMENT_KEYS = {
    'NSE': 'NSE_EQ|INE012A01025',  # ACC Ltd NSE
    'BSE': 'BSE_EQ|INE012A01025'   # ACC Ltd BSE (using ISIN)
}

class ACCDataFetcher:
    def __init__(self):
        self.headers = {
    ...
    def save_to_csv(self, df, exchange):
        ...
        filename = f"ACC_{exchange}_{FROM_DATE}_to_{TO_DATE}.csv"
        df.to_csv(filename, index=False)
        return filename

    def run(self):
        print(f"\nFetching ACC Ltd data from {FROM_DATE} to {TO_DATE}")
