...
model.load_model(r"D:\AlgoT\tabnet2_model.zip")
    ...
    if bar5==15:
        ...
        if seek_status=="Long seeking" and position=="Empty":
            ...
            if close_values[i]>=entry_target:
                ...
                dat_element.extend([(close_values[i]-close_values[seeki]),(slo_close[i]-slo_close[seeki]),( slo_ema[i]-slo_ema[seeki]),(BBB[i]-BBB[seeki]),(BBP[i]-BBP[seeki]),(MACD[i]-MACD[seeki]),(MACDh[i]-MACDh[seeki]),(MACDs[i]-MACDs[seeki]),(RSI[i]-RSI[seeki]),(ATR[i]-ATR[seeki]),(ADX[i]-ADX[seeki]),(DMP_DMN[i]-DMP_DMN[seeki]),(CCI[i]-CCI[seeki])])
                dat_elementp=np.array(dat_element)
                dat_elementp=dat_elementp.reshape(1, -1)
                prediction=(model.predict(dat_elementp))
                # print(prediction)
                pred_result=int(prediction.item())
                # print(prediction.item())
                # trade_open_time=datetime_objects[i]
                # if int(pred_result)!=1:
                if True:
                    position="Long"
                    seek_status="Empty"
                    plot_bar.append(close_values[i])
